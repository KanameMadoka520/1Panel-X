...
Progress: v1.0–v1.5 released. v1.6 monitoring [########··] Phase 16+17 (backend, CI) + Phase 18 (dashboard frontend, build:pro+ESLint clean) done; review done; release pending. v1.7 WAF-proper [#####·····] Phase 19 (real engine 10/10 CI) + Phase 20 (attack-event store, CI) + WAF attack-log frontend tab + Phase 21 gateway multi-site routing (config load/validate + Host dispatch + W12 unknown-Host default-deny + `-config` mode, CI) done; remaining Phase 21-22 = agent config generator + compose packaging + per-site nginx `proxy_pass` wiring (all human-UAT: needs live container/nginx/VPS) + Phase 23 full WAF config UI. Neither v1.6 nor v1.7 released yet. v1.5 agent binary b0fbe93f.
