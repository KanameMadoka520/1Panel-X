...
const coreOAuthMigrationTestKey = "core-unit-test-install-key"

func TestInitOneDriveDoesNotSeedSharedOAuthCredentials(t *testing.T) {
	db := openCoreOAuthMigrationDB(t)
	oldInstallDir := global.CONF.Base.InstallDir
	global.CONF.Base.InstallDir = "/opt/1panel-x-test"
	t.Cleanup(func() {
		global.CONF.Base.InstallDir = oldInstallDir
	})

	if err := InitOneDrive.Migrate(db); err != nil {
		t.Fatalf("init OneDrive settings: %v", err)
	}
	for _, key := range []string{"OneDriveID", "OneDriveSc"} {
		...
		if setting.Value != "" {
			t.Fatalf("%s was seeded with a non-empty value", key)
		}
	}
	var googleSettingCount int64
	if err := db.Model(&model.Setting{}).Where("key IN ?", []string{"GoogleID", "GoogleSc"}).Count(&googleSettingCount).Error; err != nil {
		t.Fatalf("count Google OAuth defaults: %v", err)
	}
	if googleSettingCount != 0 {
		t.Fatalf("new install created %d Google OAuth default settings, want 0", googleSettingCount)
	}

	if err := db.Model(&model.Setting{}).Where("key = ?", "OneDriveID").Update("value", "administrator-client-id").Error; err != nil {
		t.Fatalf("seed custom client ID: %v", err)
	}
	if err := db.Model(&model.Setting{}).Where("key = ?", "OneDriveSc").Update("value", "administrator-client-secret").Error; err != nil {
		t.Fatalf("seed custom client secret: %v", err)
	}
	if err := UpdateOnedrive.Migrate(db); err != nil {
		t.Fatalf("run compatibility migration: %v", err)
	}
	assertCoreSettingValue(t, db, "OneDriveID", "administrator-client-id")
	assertCoreSettingValue(t, db, "OneDriveSc", "administrator-client-secret")

	if err := MigrateBackupOAuthCredentials.Migrate(db); err != nil {
		t.Fatalf("run OAuth migration on new install: %v", err)
	}
	assertCoreSettingValue(t, db, "OneDriveID", "administrator-client-id")
	assertCoreEncryptedSettingValue(t, db, "OneDriveSc", "administrator-client-secret")
	var count int64
...
func TestMigrateBackupOAuthCredentials(t *testing.T) {
	...
	restoreLegacyFingerprints := replaceCoreLegacyOAuthFingerprints(legacyMicrosoftID, legacyMicrosoftSecret, legacyGoogleID, legacyGoogleSecret)
	t.Cleanup(restoreLegacyFingerprints)

	settings := []model.Setting{
		{Key: "OneDriveID", Value: legacyMicrosoftID},
		{Key: "OneDriveSc", Value: base64.StdEncoding.EncodeToString([]byte(legacyMicrosoftSecret))},
		{Key: "GoogleID", Value: "administrator-google-client"},
		{Key: "GoogleSc", Value: "administrator-google-secret"},
	}
	if err := db.Create(&settings).Error; err != nil {
		t.Fatalf("seed compatibility settings: %v", err)
	}

	aliyunToken := mustCoreJSON(t, map[string]interface{}{
	...
	accounts := []model.BackupAccount{
		{
			...
			Vars: mustCoreJSON(t, map[string]interface{}{
	...
	credentials := loadCoreOAuthCredentials(t, db)
	...
	assertCoreOAuthStatus(t, byAccountID[accounts[0].ID], model.BackupOAuthStatusLegacyReconfigurationRequired)
	assertCoreOAuthStatus(t, byAccountID[accounts[1].ID], model.BackupOAuthStatusLegacyReconfigurationRequired)
	assertCoreOAuthStatus(t, byAccountID[accounts[2].ID], model.BackupOAuthStatusConfigured)
	assertCoreOAuthStatus(t, byAccountID[accounts[3].ID], model.BackupOAuthStatusReauthorizationRequired)
	assertCoreOAuthStatus(t, byAccountID[accounts[4].ID], model.BackupOAuthStatusConfigured)
	...
	assertCoreEncryptedValue(t, byAccountID[accounts[0].ID].RefreshToken, "legacy-microsoft-refresh", model.BackupOAuthRefreshTokenEncryptionDomain)
	assertCoreEncryptedValue(t, byAccountID[accounts[1].ID].RefreshToken, "legacy-google-refresh", model.BackupOAuthRefreshTokenEncryptionDomain)

	customMicrosoft := byAccountID[accounts[2].ID]
	assertCoreEncryptedValue(t, customMicrosoft.ClientSecret, "administrator-microsoft-secret", model.BackupOAuthClientSecretEncryptionDomain)
	assertCoreEncryptedValue(t, customMicrosoft.RefreshToken, "administrator-microsoft-refresh", model.BackupOAuthRefreshTokenEncryptionDomain)
	...
	assertCoreEncryptedValue(t, customGoogle.ClientSecret, "administrator-google-secret", model.BackupOAuthClientSecretEncryptionDomain)
	...
	assertCoreEncryptedValue(t, aliyun.RefreshToken, "aliyun-refresh-token", model.BackupOAuthRefreshTokenEncryptionDomain)

	for _, account := range accounts {
		vars := loadCoreBackupVars(t, db, account.ID)
	...
	assertCoreSettingValue(t, db, "OneDriveID", "")
	assertCoreSettingValue(t, db, "OneDriveSc", "")
	assertCoreSettingValue(t, db, "GoogleID", "administrator-google-client")
	assertCoreEncryptedSettingValue(t, db, "GoogleSc", "administrator-google-secret")
	...
	afterRerun := loadCoreOAuthCredentials(t, db)
...
func TestKnownLegacyOAuthSettingSupportsRawAndBase64(t *testing.T) {
	const (
		legacyMicrosoftID     = "synthetic-setting-microsoft-client"
		legacyMicrosoftSecret = "synthetic-setting-microsoft-secret"
		legacyGoogleID        = "synthetic-setting-google-client"
		legacyGoogleSecret    = "synthetic-setting-google-secret"
	)
	restore := replaceCoreLegacyOAuthFingerprints(legacyMicrosoftID, legacyMicrosoftSecret, legacyGoogleID, legacyGoogleSecret)
	t.Cleanup(restore)
	...
	tests := []struct {
		...
		{key: "OneDriveID", value: legacyMicrosoftID},
		{key: "OneDriveSc", value: base64.StdEncoding.EncodeToString([]byte(legacyMicrosoftSecret))},
		{key: "GoogleID", value: base64.StdEncoding.EncodeToString([]byte(legacyGoogleID))},
		{key: "GoogleSc", value: legacyGoogleSecret},
	}
	for _, tt := range tests {
		if !isKnownLegacyOAuthSetting(tt.key, tt.value) {
			t.Fatalf("%s synthetic legacy value was not recognized", tt.key)
		}
	}
	if isKnownLegacyOAuthSetting("GoogleID", "administrator-owned-client") {
		t.Fatal("administrator-owned setting was misclassified as legacy")
	}
}
...
func TestKnownLegacySecretDoesNotRequireMatchingClientID(t *testing.T) {
	...
	legacyOAuthFingerprints = map[string][]legacyOAuthFingerprint{
		model.BackupOAuthProviderMicrosoft: {{clientSecret: sha256Hex([]byte("synthetic-retired-secret"))}},
	}
...
func openCoreOAuthMigrationDB(t *testing.T) *gorm.DB {
	t.Helper()
	...
	if err := db.Create(&model.Setting{Key: "EncryptKey", Value: coreOAuthMigrationTestKey}).Error; err != nil {
...
func replaceCoreLegacyOAuthFingerprints(microsoftID, microsoftSecret, googleID, googleSecret string) func() {
	old := legacyOAuthFingerprints
	legacyOAuthFingerprints = map[string][]legacyOAuthFingerprint{
		model.BackupOAuthProviderMicrosoft: {
			{clientID: sha256Hex([]byte(microsoftID)), clientSecret: sha256Hex([]byte(microsoftSecret))},
		},
		model.BackupOAuthProviderGoogle: {
			{clientID: sha256Hex([]byte(googleID)), clientSecret: sha256Hex([]byte(googleSecret))},
		},
	}
	return func() {
...
func mustCoreJSON(t *testing.T, value interface{}) string {
	t.Helper()
...
func loadCoreOAuthCredentials(t *testing.T, db *gorm.DB) []model.BackupOAuthCredential {
	t.Helper()
...
func loadCoreBackupVars(t *testing.T, db *gorm.DB, accountID uint) map[string]interface{} {
	t.Helper()
...
func assertCoreOAuthStatus(t *testing.T, credential model.BackupOAuthCredential, want string) {
	t.Helper()
...
func assertCoreEncryptedValue(t *testing.T, ciphertext, plaintext, domain string) {
	t.Helper()
	if ciphertext == "" {
	...
	decrypted, err := encrypt.StringDecryptGCMWithKey(ciphertext, coreOAuthMigrationTestKey, domain)
	if err != nil {
...
func assertCoreSettingValue(t *testing.T, db *gorm.DB, key, want string) {
	t.Helper()
	var setting model.Setting
	if err := db.Where("key = ?", key).First(&setting).Error; err != nil {
	...
	if setting.Value != want {
		t.Fatalf("setting %s = %q, want %q", key, setting.Value, want)
	}
}

func assertCoreEncryptedSettingValue(t *testing.T, db *gorm.DB, key, plaintext string) {
	t.Helper()
	var setting model.Setting
	if err := db.Where("key = ?", key).First(&setting).Error; err != nil {
		t.Fatalf("load encrypted setting %s: %v", key, err)
	}
	assertCoreEncryptedValue(t, setting.Value, plaintext, model.BackupOAuthLegacySettingEncryptionDomain)
}
