...
var MigrateBackupOAuthCredentials = &gormigrate.Migration{
	...
	Migrate: func(tx *gorm.DB) error {
		...
		if err := migrateBackupOAuthAccounts(tx); err != nil {
			return err
		}
		return clearKnownLegacyOAuthSettings(tx)
	},
}
...
func clearKnownLegacyOAuthSettings(tx *gorm.DB) error {
	encryptionKey, err := loadBackupOAuthEncryptionKey(tx)
	if err != nil {
		return err
	}
	for _, key := range []string{"OneDriveID", "OneDriveSc", "GoogleID", "GoogleSc"} {
		var setting model.Setting
		...
		if isKnownLegacyOAuthSetting(key, setting.Value) {
			if err := tx.Model(&model.Setting{}).Where("id = ?", setting.ID).Update("value", "").Error; err != nil {
				return err
			}
			continue
		}
		if strings.HasSuffix(key, "ID") || strings.HasPrefix(setting.Value, "enc:gcm:v1:") {
			continue
		}
		plain := setting.Value
		if decoded, decodeErr := base64.StdEncoding.DecodeString(setting.Value); decodeErr == nil {
			plain = string(decoded)
		}
		encrypted, err := encrypt.StringEncryptGCMWithKey(plain, encryptionKey, model.BackupOAuthLegacySettingEncryptionDomain)
		if err != nil {
			return err
		}
		if err := tx.Model(&model.Setting{}).Where("id = ?", setting.ID).Update("value", encrypted).Error; err != nil {
			return err
		}
	}
	return nil
...
func isKnownLegacyOAuthSetting(key, value string) bool {
	provider := model.BackupOAuthProviderMicrosoft
	clientID := strings.HasSuffix(key, "ID")
	if strings.HasPrefix(key, "Google") {
		provider = model.BackupOAuthProviderGoogle
	}
	for _, fingerprint := range legacyOAuthFingerprints[provider] {
		expected := fingerprint.clientSecret
		if clientID {
			expected = fingerprint.clientID
		}
		if valueMatchesFingerprint(value, expected) {
			return true
		}
	}
	return false
...
func valueMatchesFingerprint(value, expected string) bool {
	if fingerprintsEqual(sha256Hex([]byte(value)), expected) {
		return true
	}
	...
	return fingerprintsEqual(sha256Hex(decoded), expected)
}
