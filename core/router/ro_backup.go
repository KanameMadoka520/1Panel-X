...
func (s *BackupRouter) InitRouter(Router *gin.RouterGroup) {
	...
	baseApi := v2.ApiGroupApp.BaseApi
	{
		...
		backupRouter.GET("/sync/status", baseApi.ListBackupSyncStatus)
		backupRouter.GET("/sync/status/:name", baseApi.GetBackupSyncStatus)
		backupRouter.POST("/sync/retry", baseApi.RetryBackupSync)
